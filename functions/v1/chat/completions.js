...
export async function onRequestPost(context) {
  ...
  try {
    ...
    // candidates 是 { provider, resolvedModel } 对象数组
    let candidates;
    if (pinnedProvider) {
      candidates = [{ provider: pinnedProvider, resolvedModel: resolvedModel }];
    } else {
    ...
    if (isStream) {
      ...
      for (let attempt = 0; attempt < maxStreamRetries; attempt++) {
        ...
        try {
          const streamResp = await handleStreamRequest(upstreamUrl, upstreamHeaders, upstreamBody, provider.type, actualModel);
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
      ...
      if (provider.type === "anthropic") {
        result = convertAnthropicResponse(respData, actualModel);
      } else if (provider.type === "google") {
        result = convertGoogleResponse(respData, actualModel);
      } else if (provider.type === "cohere") {
        result = convertCohereResponse(respData, actualModel);
      } else {
        result = respData;
      }
      ...
      result._gateway = {
        ...
        latency,
        attempt: attempt + 1,
        ...(aliasResult ? { originalModel: model, actualModel } : {}),
      };
